...
def _write_stub_agent_framework(root: Path) -> None:
    ...
    package_dir.mkdir()
    mcp_package_dir = root / "mcp"
    mcp_package_dir.mkdir()
    (mcp_package_dir / "__init__.py").write_text("", encoding="utf-8")
        ...
        textwrap.dedent(
            ...
            class Agent:
                ...
                async def run(self, prompt, *, options=None, **kwargs):
                    ...
                    if self.name == "Config-Synthesizer":
                        return Result(
                            '{"kind":"agent","agent":{"name":"Status Writer","description":"Writes concise updates.","prompt":"Write concise status updates from the user request."}}'
                        )
        ...
        textwrap.dedent(
            ...
            class Workflow:
                ...
                def run(self, prompt, *, stream=False):
                    ...
                    if "builder" in self._team_name().lower() or any(
                        "config-synthesizer" in participant_name.lower() for participant_name in participant_names
                    ):
                        final_text = (
